...
- De novo assemblies (fasta format)
- Raw sequencing reads (fastq format) from Illumina or Oxford Nanopore (ONT) sequencers
...
When provided with the raw sequencing data for a sample (a pair of fastq files for paired-end Illumina data, or a single fastq file for single-end Illumina or ONT data), Vibecheck:

1. Subsamples 20% of reads using [seqtk](https://github.com/lh3/seqtk) to ease computational demands and speed-up the analysis.
2. Aligns reads to a reference O1 _V. cholerae_ genome using [minimap2](https://github.com/lh3/minimap2), using the `sr` preset for Illumina reads or the `map-ont` preset for ONT reads.
...
vibecheck [query ...]
```
Where `[query ...]` is the name of your input fasta file, a pair of fastq files, or a single fastq file. 
A query fasta file can contain as many sequences as you would like to be classified, while fastq files should 
only contain the data for one sample. 

By default, Vibecheck assumes each sequence in a fasta file comes from a separate sample, if the sequences 
instead represent contigs from a single _de novo_ assembly, add `--assemblies`.

Vibecheck assumes fastq files were generated by an Illumina sequencer. To classify Oxford Nanopore (ONT) reads, supply a single fastq file and add `--platform ont`:
```commandline
vibecheck --platform ont reads.fastq.gz
```
...
> The sequence-based classification workflow is available [here](https://dockstore.org/workflows/github.com/CholGen/Vibecheck/vibecheck-lineage-assignment:main), 
> and the read-based classification workflow is available [here](https://dockstore.org/workflows/github.com/CholGen/Vibecheck/vibecheck-lineage-assignment-reads:main)

## Full usage options
```
usage: vibecheck [-h] [-o OUTDIR] [--outfile OUTFILE] [--tempdir TEMPDIR] [--no-temp] [--lineage-aliases LINEAGE_ALIASES] [-t THREADS] [-v] [-u USHER_TREE] [-m MAX_AMBIGUITY] [--assemblies] [-b BARCODES] [--platform {illumina,ont}] [-s SUBSAMPLE] [--no-subsample] [query ...]
...
options:
  ...
  --lineage-aliases LINEAGE_ALIASES
                        Path to CSV file containing lineage aliases. File must have two columns: `alias` and `lineage`.
...
Sequence-based classification:
  ...
  --assemblies          Treat the input FASTA as a set of contigs from a single assembled sample, and produce one classification for the whole sample instead of one per sequence.

Read-based classification:
  -b BARCODES, --barcodes BARCODES
                        Feather formatted lineage barcodes to use instead of default O1 barcodes
  --platform {illumina,ont}
                        Sequencing platform used to generate the reads. Default: illumina
